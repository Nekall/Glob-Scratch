  ...
  return (
    <Svg xmlns="http://www.w3.org/2000/svg" width="907" height="1000">
      <Path
        ...
        stroke="#CBA365"
        strokeWidth="1.5"
        d="M783 1000v-130l119-78"
      />
      {arrDpt.map((dpt, index) => {
        return (
          <Path
            key={`dpt-${index}`}
            id={dpt.id}
            fill={departments.includes(dpt.id) ? '#141311' : '#CBA365'}
            stroke={departments.includes(dpt.id) ? '#CBA365' : '#141311'}
            strokeWidth="0.5"
            d={dpt.d}
            onPress={() =>
              Alert.alert(
                departments.includes(dpt.id) ? 'Réinitialiser' : 'Gratter',
                departments.includes(dpt.id)
                  ? `Voulez vous réinitialiser le ${dpt.id}`
                  : `Voulez vous gratter le ${dpt.id}`,
                [
                  {
                    text: 'Confirmer',
                    onPress: () =>
